...
def main(args):
    assert args.variant in [
        "gold", "gold_limit1000", # limit test to 1000 examples
        "random", # main experiments in Section 4,
        ...
        "random_english_words_gold_labels", "permutated_labels","permutated_train_labels", "random_true_distribution"
    ]
    ...
    for dataset_idx, (dataset, new_dataset) in enumerate(zip(datasets, new_datasets)):
        ...
        for seed in seeds:
            ...
            # Limit the size of test_data to 1000 examples using local RNG
            local_rng = np.random.RandomState(1234)  # Replace YOUR_SEED_VALUE with your desired seed number
            if len(test_data) > 1000:
                test_data = list(local_rng.choice(test_data, 1000, replace=False))
            
            if args.variant == "gold_limit1000":
                ...
                print ("Done gold limit 1000 for %s seed=%s" % (new_dataset, seed))
                continue 
            ...
            # added by Zhuoyan
            # modify only train input for permutated_labels with classification tasks
            if args.variant == "permutated_train_labels" and config["task_type"]=="classification":
                old_options = config["options"]
                new_options = [old_options[(i+1)%len(old_options)] for i in range(len(old_options))]
                new_mapping = {old_option: new_option for old_option, new_option in zip(old_options, new_options)}
            ...
            for dp_idx, dp in enumerate(train_data):
                if args.variant in ["gold", "gold_w_template", "permutated_labels","permutated_train_labels", "random_english_words_gold_labels"] or \
